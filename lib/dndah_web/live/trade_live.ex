defmodule DndahWeb.TradeLive do
  ...
  alias Dndah.Market
  alias Dndah.Market.Events.TradeMessage
    ...
    case Market.find_listing_offer(socket.assigns.current_user, id) do
      {:ok, offer} ->
        if connected?(socket) do
          ...
          DndahWeb.Presence.subscribe(offer)
          DndahWeb.Presence.track_trade(offer, socket.assigns.current_user)
        end
  ...
  def handle_info({DndahWeb.Presence, {:user_joined, presence}}, socket) do
    {:noreply, assign_presence(socket, presence)}
  end

  def handle_info({DndahWeb.Presence, {:user_left, presence}}, socket) do
  ...
  defp assign_presences(socket) do
    ...
    if offer = connected?(socket) && socket.assigns.offer do
      offer
      |> DndahWeb.Presence.list_trade_users()
